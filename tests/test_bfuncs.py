...
from miexp.bfuncs import (
    MajDataset,
    MultiComponentDNFDataset,
    MultiComponentSpectrumDataset,
)
...
def test_multicomponentspectrumdataset_getitem():
    ...
    def test_multicomponentdnfdataset_init():
        N = 5
        num_components = 3
        num_samples = 1000
        comps = torch.randint(0, 2, (num_components, N)).type(torch.int)
        dataset = MultiComponentDNFDataset(N, comps, num_samples)
        ...
        assert torch.equal(
            dataset.labels,
            dataset.generate_labels(dataset.data, comps).type(torch.int),
        )

    def test_multicomponentdnfdataset_default_num_samples():
        N = 5
        num_components = 3
        comps = torch.randint(0, 2, (num_components, N)).type(torch.int)
        dataset = MultiComponentDNFDataset(N, comps)

        assert dataset.num_samples == 10000
        assert dataset.data.shape == (10000, N)
        assert dataset.labels.shape == (10000,)

    def test_multicomponentdnfdataset_len():
        N = 5
        num_components = 3
        num_samples = 1000
        comps = torch.randint(0, 2, (num_components, N)).type(torch.int)
        ...
        default_dataset = MultiComponentDNFDataset(N, comps)
        assert len(default_dataset) == 10000

    def test_multicomponentdnfdataset_getitem():
        N = 5
        num_components = 3
        num_samples = 1000
        comps = torch.randint(0, 2, (num_components, N)).type(torch.int)
