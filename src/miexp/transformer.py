...
class Transformer(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Embedding(
            num_embeddings=P + 1, embedding_dim=embed_dim, device=device
        )
        self.pos_embedding = nn.Embedding(
            num_embeddings=num_summands + 1, embedding_dim=embed_dim, device=device
        )
    ...
    def forward(self, inputs: Tensor):
        ...
        embeds = self.embedding(inputs) + self.pos_embedding(
            torch.arange(self.num_summands + 1, device=self.device)
        )
        # embeds shape: (B, N, E)
                    ...
                    torch.matmul(
                        ...
                        torch.transpose(
                            Q[
                                :,
                                0:1,
                                j * self.per_head_dim : (j + 1) * self.per_head_dim,
                            ],
                            -1,
                            -2,
                        ),
            ...
            torch.concat(
                [
                    torch.matmul(
                        ...
                        V[:, :, j * self.per_head_dim : (j + 1) * self.per_head_dim],
                    ).view(-1, self.per_head_dim)
